...
void * calSeqRW(void *param)
{
    ...
    for (long i=0; i<loops; i++) {
        memcpy(buffer+blockSize*((i*2)%blockNum),buffer+blockSize*((i*2+1)%blockNum),blockSize);
    }
    pthread_exit(NULL);
}
...
void * calRanRW(void *param)
{
    ...
    for (long i=0; i<loops; i++) {
        ...
        memcpy(buffer+blockSize*((i+rand())%blockNum),buffer+blockSize*((i+rand())%blockNum),blockSize);
    }
    pthread_exit(NULL);
}
...
void * calSeqLat(void *param)
{
    ...
    //    req.tv_sec=0;
    for (long i=0; i<loops; i++) {
        memcpy(buffer+((i*2*128)%blockNum),buffer+(((i*2+1)*128)%blockNum),1);  //(i*128) to deal with cache line size
        //        nanosleep(&req, &rem);
    }
    pthread_exit(NULL);
}
...
void * calRanLat(void *param)
{
    ...
    for (long i=0; i<loops; i++) {
        ...
        memcpy(buffer+((i+rand())%blockNum),buffer+((i+rand())%blockNum),1);
    }
    pthread_exit(NULL);
}
...
int main(int argc, const char * argv[])
{
    ...
    int iterNum;
    char *buffer;                                  //first buffer. The source of memcpy
    ...
    for (int i=0; i<numOfThread; i++) {
        paramForMemoryTest[i].buffer=buffer+i*bufferSize*2/numOfThread;
        paramForMemoryTest[i].bufferSize=bufferSize*2/numOfThread;
